...
import com.huarong.p2p.api.mybatis.mapper.SmsConfigMapper;
...
public class SmsConfigServiceImpl implements SmsConfigService {
	...
	private static Logger logger = LoggerFactory
			.getLogger(SmsConfigServiceImpl.class);
	 ...
	 * @Throws:
	 */
	public  Response sendSMS(String userName, String password,
			String content, String phone, String randomCode, String pathtype)
			throws SQLException {

		logger.debug("------------code------" + randomCode);
		Response response = sendHrSms(content, phone, randomCode, pathtype);

		return response;

	}

	public Response sendHrSms(String content, String phone, String randomCode,
			String pathtype) {
		Response response = new Response().failure("发送失败");//默认发送失败
		...
		SmsConfig smsConfig = smsConfigMapper.selectByCondition(condition);
		smsuser = username;
		smspwd = password;
		smsendpoint = endpoint;
		if (smsConfig != null) {// 重置短息接口渠道
		...
		try {
			...
			Select select = sysparService.selectBySelectKey("selectValue",
					"SMSACOUNT");
			if (select != null) {
				int useacount = select.getSelectvalue();// 可发数量
				int smsacount = sendSmsService.checkSendSmsCount(beginTime,
						endTime, phone);// 已发数量
				if (smsacount >= useacount) {
					logger.debug("~~~~~~~~~~~~~~~~~~~~~~~~~~发送已上限~~~~~~~~~~~~~~~~~~~~~~~");
					response.failure("短信发送已上限");
					return response;
				}
			}
			// 插入数据库
		...
		logger.debug("smsuser:" + smsuser + ";smspwd:" + smspwd
		...
		// 加入亿美短信发送,亿美短信已未使用
		/*
		 * if (businessId==1) { Client client =
		 * SingletonClient.getClient(smsuser, smspwd);
		 * log.info("【华融金融】"+content); int code = client.sendSMS(new String[] {
		 * phone }, "【华融金融】"+content, "",5);
		 * log.info(code==0?"亿美短信发送成功！":"亿美短信发送失败!"); return code+""; }
		 */
		// 新加的短信方式 http://58.83.147.92:8080/qxt/smssenderv2

		/*
		 * org.apache.cxf.service.Service service = new Service(); Call call =
		 * null;
		 * 
		 * call = (Call) service.createCall();
		 * 
		 * //采用AZDG方式加密 //采用AZDG方式加密
		 * 
		 * //发送短信 call.setOperationName("InsertDownSms");
		 * call.setTargetEndpointAddress(new java.net.URL(smsendpoint)); String
		 * boay = "<sendbody><message><orgaddr></orgaddr><mobile>"+phone+
		 * "</mobile><content>" + content +
		 * "</content><sendtime></sendtime><needreport>0</needreport></message><publicContent></publicContent></sendbody>"
		 * ; String ret1 = (String) call.invoke(new Object[] { smsuser, smspwd,
		 * "", boay }); ret1.length(); JSONObject jsonObj =
		 * XML.toJSONObject(ret1); JSONObject response = (JSONObject)
		 * jsonObj.get("response"); JSONObject head = (JSONObject)
		 * response.get("head"); String code = (String) head.get("code");
